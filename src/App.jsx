...
function PanelLocales({locales, localesDatos, localesObras, recetas, usuario, onSaveDatos, onSaveObra, onDeleteObra, onSaveEgreso, onSaveReceta, onDeleteReceta}){
  ...
  function doSaveObra(){
    ...
    if(pagosValidos.length>0&&totalObraVal>0&&onSaveEgreso&&formObra.estado!=="pendiente"){
...
function PanelInforme({gastos, sueldos, cargasSociales, localesObras, empleados}){
  ...
  var mesesDisp=[...new Set([
    ...
    ...localesObras.map(function(o){return o.fecha?o.fecha.slice(0,7):null;})
  ].filter(Boolean))].sort().reverse();
  if(mesesDisp.indexOf(mesCurrent)===-1)mesesDisp.unshift(mesCurrent);

  // Filtrar por mes y local
  function filtrarLocal(items, localField){
    var byMes=items.filter(function(x){
      var fecha=x.fecha||x.periodo||"";
      return fecha.slice(0,7)===mesFiltro;
    });
    if(localFiltro==="all")return byMes;
    return byMes.filter(function(x){return(x[localField]||x.local)===localFiltro;});
  }

  // Sueldos del mes
  var sueldosMes=filtrarLocal(sueldos.filter(function(s){return s.estado!=="pendiente";}), "local");
  var totalSueldos=sueldosMes.reduce(function(a,s){return a+parseFloat(s.monto||0);},0);

  // F.931 del mes (gastos con area F.931)
  var f931Mes=filtrarLocal(gastos.filter(function(g){return g.area==="F.931";}), "local");
  var totalF931=f931Mes.reduce(function(a,g){return a+parseFloat(g.monto||0);},0);

  // Obras del mes
  var obrasMes=localesObras.filter(function(o){
    var ok=o.fecha&&o.fecha.slice(0,7)===mesFiltro&&o.estado!=="pendiente";
    if(localFiltro!=="all")ok=ok&&o.local===localFiltro;
    return ok;
  });
  var totalObras=obrasMes.reduce(function(a,o){return a+(parseFloat(o.mano_obra)||0)+(parseFloat(o.materiales)||0)+(parseFloat(o.servicios)||0);},0);

  var totalGeneral=totalSueldos+totalF931+totalObras;
  ...
  return(
    <div style={{fontFamily:"'Inter',sans-serif"}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8,marginBottom:14}}>
        {[["👥 Sueldos",totalSueldos,"#4CAF50"],["🏛️ F.931",totalF931,"#4CAF50"],["🏗️ Obras",totalObras,"#E07B00"]].map(function(t){return(
          <div key={t[0]} style={{background:"#111",border:"1px solid "+t[2]+"33",borderRadius:10,padding:"10px 12px",textAlign:"center"}}>
            <div style={{fontSize:10,color:t[2],marginBottom:4}}>{t[0]}</div>
            <div style={{fontSize:16,fontWeight:800,color:t[2],fontFamily:"'Playfair Display',serif"}}>{fmt(t[1])}</div>
          </div>
        );})}
      </div>
      <div style={{background:"#111",border:"1px solid #D4A01733",borderRadius:10,padding:"12px",marginBottom:14,textAlign:"center"}}>
        <div style={{fontSize:10,color:"#D4A017",marginBottom:4}}>Total del mes</div>
        <div style={{fontSize:22,fontWeight:800,color:"#D4A017",fontFamily:"'Playfair Display',serif"}}>{fmt(totalGeneral)}</div>
      </div>
      ...
      {sueldosMes.length>0&&(
        <div style={{background:"#0F0F0F",border:"1px solid #4CAF5022",borderRadius:12,padding:"14px",marginBottom:10}}>
          ...
          {localesPrincipales.map(function(l){
      ...
      {/* Obras */}
      {obrasMes.length>0&&(
        <div style={{background:"#0F0F0F",border:"1px solid #E07B0022",borderRadius:12,padding:"14px",marginBottom:10}}>
          <div style={{fontSize:11,fontWeight:700,color:"#E07B00",marginBottom:10}}>🏗️ Obras — {fmt(totalObras)}</div>
          {obrasMes.map(function(o){
            var tot=(parseFloat(o.mano_obra)||0)+(parseFloat(o.materiales)||0)+(parseFloat(o.servicios)||0);
            var loc=LOCALES.find(function(l){return l.id===o.local;});
            return(
              <div key={o.id} style={{marginBottom:8,padding:"6px 0",borderBottom:"1px solid #111"}}>
...
function PlanillaInline({empleados, planilla, sueldos, adelantos, onSave, onDelete}){
  ...
  function abrirModal(emp,mes){
    ...
    if(plan){
      ...
      setModalForm({tipo,monto_convenio:String(plan.monto_convenio||""),monto_sin_convenio:String(plan.monto_sin_convenio||plan.monto||""),esAguinaldo});
    } else {
      setModalForm({tipo:"sin_convenio",monto_convenio:"",monto_sin_convenio:"",esAguinaldo});
    }
    setShowModal(true);
  }
  ...
  function doGuardar(){
    ...
    var obj={
      ...
      empleado_nombre:modalEmp.nombre,
      local:modalEmp.local,
      anio,
      mes:esAguinaldo?null:modalMes,
      tipo:esAguinaldo?modalMes:modalForm.tipo,
      monto:total,
      monto_convenio:modalForm.tipo==="convenio"||modalForm.tipo==="mixto"?conv:0,
      monto_sin_convenio:modalForm.tipo==="sin_convenio"||modalForm.tipo==="mixto"?sinc:0
    };
    if(onSave)onSave(obj);
    setShowModal(false);
  }
...
function PanelEgresosSueldos({planillaSueldos, sueldos, empleados, gastos, usuario, onSaveSueldo, onSaveEgresoSueldo, onDeleteSueldo, p}){
  ...
  function pagosAdelanto(){
    var total=parseFloat(adelantoForm.monto)||0;
    var filas=(adelantoForm.pagos||[]).filter(function(pg){return pg.medio;});
    if(filas.length===1&&!(parseFloat(filas[0].monto)>0))return [{medio:filas[0].medio,monto:total}];
  ...
  function totalPagosAdelanto(){return pagosAdelanto().reduce(function(a,pg){return a+pg.monto;},0);}
  function pagosAdelantoCuadran(){var t=parseFloat(adelantoForm.monto)||0;return t<=0||Math.abs(totalPagosAdelanto()-t)<0.01;}
  function setPagoAdelanto(idx,campo,valor){
  ...
  function doGuardarAdelanto(){
    ...
    var a={
      ...
      medio_pago:pagosValidos[0].medio||"",
      pagos:pagosValidos,
  ...
  // ── Medios de pago del modal ──
  // El pago se puede repartir entre varias cuentas. Los montos se cargan sobre el
  // neto que realmente sale ahora (ya descontados los adelantos). Si hay un solo
  // medio y no se escribe monto, se asume que cubre todo el neto.
  function netoModal(){
    if(!modalPl)return 0;
  ...
  function totalPagosModal(){return pagosModal().reduce(function(a,pg){return a+pg.monto;},0);}
  function pagosModalCuadran(){var n=netoModal();return n<=0||Math.abs(totalPagosModal()-n)<0.01;}
  function setPagoModal(idx,campo,valor){
    setModalForm(function(f){
      var n=[...(f.pagos||[])];
  ...
  function doGuardar(){
    ...
    var pagoExistente=sueldosMes.find(function(s){
      return s.empleado_id===modalPl.empleado_id&&(esAguinaldo?s.concepto_extra===modalPl.tipo:!s.concepto_extra);
    });
    var s={
                ...
                <div style={{marginBottom:8}}>
                  ...
                  {(adelantoForm.pagos||[]).map(function(pago,idx){return(
                    <div key={idx} style={{display:"flex",gap:6,marginBottom:6,alignItems:"center"}}>
                      <select value={pago.medio} onChange={function(e){setPagoAdelanto(idx,"medio",e.target.value);}} style={{...INP,fontSize:11}}>
                        <option value="">-- Seleccioná --</option>
                ...
                <div style={{background:"#1A140A",border:"1px solid #D4A01744",borderRadius:8,padding:"8px 10px",marginBottom:12}}>
                  ...
                  {adelPend.map(function(a){return(
                    <div key={a.id} style={{display:"flex",justifyContent:"space-between",fontSize:10,color:"#888"}}>
                      <span>{fmtFechaCorta(a.fecha)} · {textoMedios(a)}</span>
...
var DEP_TIPOS={
  ...
  // Un turno es alquilar la cancha a quien venga; esto es lo que paga un profe por las
  // horas que usa para sus clases, a su tarifa. Entra plata en los dos casos, pero por
  // motivos distintos, y mezclarlos haría imposible saber de dónde viene cada peso.
  uso:{label:"🤝 Horas de profes",singular:"uso",articulo:"el",color:"#8B2FC9",vacio:"Sin horas de profes cargadas",ok:"cobrado"},
};
...
var DEP_CAMPOS={
  ...
  uso:[
    {k:"fecha",     label:"Fecha",         tipo:"date",  req:true},
    {k:"nombre",    label:"Profe",         tipo:"text",  req:true, ph:"Quién usó la cancha", sug:"profes"},
    {k:"cantidad",  label:"Horas usadas",  tipo:"num",   ph:"1"},
    {k:"precio",    label:"$ por hora",    tipo:"num",   ph:"0"},
    {k:"monto",     label:"Total $",       tipo:"num",   ph:"0"},
    {k:"medio_pago",label:"Medio de pago", tipo:"select",opciones:DEP_MEDIOS},
  ],
  obra:[
    {k:"fecha",     label:"Fecha",         tipo:"date",  req:true},
...
function depTiposDe(seccionId){
  if(seccionId==="entradas")return ["entrada","clase","turno","uso"];
...
var DEP_ORIGENES=[
  ...
  {id:"uso",        label:"🤝 Horas de un profe",desc:"Lo que paga por las que usó", disciplina:"caja", tipo:"uso"},
...
function depQueEs(x){
  ...
  if(x.tipo==="uso")return "🤝 Horas de profe";
...
function PanelDeportes(p){
  ...
  function set(k,v){
    setForm(function(prev){
      ...
      if(fTipo==="uso"){
        // Al elegir el profe se trae su tarifa de la ficha, que es donde vive el precio
        // por hora: tenerlo en un solo lugar evita que cada carga diga una cosa distinta.
        if(k==="nombre"&&!String(n.precio||"").trim()){
          var ficha=registros.find(function(x){
            return x.tipo==="profe"&&String(x.nombre||"").trim().toLowerCase()===String(v||"").trim().toLowerCase();
          });
          if(ficha&&ficha.precio)n.precio=String(ficha.precio);
        }
        var h=parseFloat(n.cantidad), pr=parseFloat(n.precio);
        if(!isNaN(h)&&!isNaN(pr))n.monto=String(Math.round(h*pr));
      }
      return n;
    });
  }
  ...
  var totalEntradas=registros.filter(function(x){
    return (x.tipo==="entrada"||x.tipo==="clase"||x.tipo==="turno"||x.tipo==="uso")&&x.estado==="cobrado"&&enMes(x);
...
// cuentas: una parte en efectivo del local y el resto por transferencia, por ejemplo.
...
function mediosDe(item){
  if(!item)return [];
  var lista=(item.pagos||[]).filter(function(pg){return (pg.medio||pg.tipo)&&(parseFloat(pg.monto)||0)>0;});
  if(lista.length>0)return lista.map(function(pg){return {medio:pg.medio||pg.tipo,monto:parseFloat(pg.monto)||0};});
  var unico=item.medio_pago||item.forma_pago||"";
  return unico?[{medio:unico,monto:parseFloat(item.monto)||0}]:[];
}

// Reparte `monto` entre los medios del registro devolviendo siempre [{medio, monto}]
// cuyo total es exactamente `monto`. Si los medios cargados no suman lo mismo
// (por ejemplo porque se descontaron adelantos) se prorratea, así ningún total
// del sistema cambia por el hecho de haber dividido el pago.
function repartirMedios(item,monto){
  var total=parseFloat(monto)||0;
  var lista=mediosDe(item);
  if(lista.length===0)return [{medio:"",monto:total}];
  if(lista.length===1)return [{medio:lista[0].medio,monto:total}];
  var suma=lista.reduce(function(a,pg){return a+pg.monto;},0);
  if(suma<=0)return [{medio:lista[0].medio,monto:total}];
  var out=lista.map(function(pg){return {medio:pg.medio,monto:total*pg.monto/suma};});
  var acum=out.reduce(function(a,pg){return a+pg.monto;},0);
  if(Math.abs(acum-total)>0.004)out[out.length-1].monto+=total-acum;
  return out;
}

// Texto corto para mostrar el/los medios de un registro.
function textoMedios(item){
  var lista=mediosDe(item);
        ...
        <div style={{position:"fixed",top:0,left:0,right:0,bottom:0,background:"#0A0A0A",zIndex:999,overflowY:"auto",padding:"16px",fontFamily:"'Inter',sans-serif"}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:10,alignItems:"start"}}>
            {LOCALES.filter(function(l){return l.id!=="l4";}).map(function(l){
              ...
              gl.forEach(function(g){
                var cat=(function(){
                  var a=(g.area&&g.area.trim())||"";
                  var c=(g.categoria&&g.categoria.trim())||"";
                  if(a&&a!=="Proveedores")return a.split(" - ")[0];
                  if(c&&AREAS_BASE.includes(c.split(" - ")[0]))return c.split(" - ")[0];
...
function PanelRetiros(p) {
  ...
  var filtered=retiros.filter(function(r){
    ...
    if(filtroFecha==="mes") matchFecha=r.fecha&&r.fecha.slice(0,7)===mesFiltro;
    return matchLocal&&matchFecha;
  });
      ...
      <div style={{display:"flex",gap:5,marginBottom:13,flexWrap:"wrap",alignItems:"center"}}>
        ...
        {filtroFecha==="mes"&&(
          <select value={mesFiltro} onChange={function(e){setMesFiltro(e.target.value);}} style={{padding:"3px 8px",borderRadius:8,border:"1px solid #2A2A2A",background:"#111",color:"#8B2FC9",fontFamily:"'Inter',sans-serif",fontSize:11,cursor:"pointer"}}>
            {mesesDisponibles.map(function(m){return <option key={m} value={m}>{m}</option>;})}
          </select>
        )}
        <div style={{width:1,height:16,background:"#222",margin:"0 4px"}}/>
        {LOCALES.map(function(l){return(
...
function PanelAportes(p) {
  ...
  var mesesDisponibles=[...new Set([hoy.slice(0,7),...aportes.map(function(a){return a.fecha?a.fecha.slice(0,7):null;}).filter(Boolean)])].sort().reverse();
  ...
  var filtered=aportes.filter(function(a){
    ...
    if(filtroFecha==="mes") matchFecha=a.fecha&&a.fecha.slice(0,7)===mesFiltro;
    return matchLocal&&matchFecha;
  });
...
function comisionCargadaAMano(gastos, lid, mes){
  return (gastos||[]).filter(function(g){
    return g.local===lid&&g.fecha&&g.fecha.substring(0,7)===mes&&esGastoComision(g);
  }).reduce(function(a,g){return a+parseFloat(g.monto||0);},0);
}
function comisionDeCierres(cierres, lid, mes){
  return (cierres||[]).filter(function(c){
    return c.local===lid&&c.fecha&&c.fecha.substring(0,7)===mes;
  }).reduce(function(a,c){return a+comisionDeCierre(c);},0);
}
...
function PanelResultados(p){
      ...
      <div style={{background:"#080808",border:"1px solid #1A1A1A",borderRadius:8,padding:"10px 12px",marginTop:6,marginBottom:6}}>
        ...
        {tipo==="electronico"&&(d.comisionElectronico||0)>0&&(
          <div style={{display:"flex",justifyContent:"space-between",fontSize:10,color:"#8A6A2A",marginTop:6,paddingTop:6,borderTop:"1px solid #1A1A1A"}}>
            ...
            <span style={{fontWeight:600}}>−{fmt(d.comisionElectronico)}</span>
          </div>
        )}
  ...
  function calcLocal(lid){
    ...
    sueldosADescontar.forEach(function(s){
      ...
      // Un sueldo se puede haber pagado con varios medios: cada parte va a su caja.
      repartirMedios(s,sm).forEach(function(pg){
        var esEf=(pg.medio||"").toLowerCase().includes("efectivo");
        if(esEf)gastoEfectivo+=pg.monto;else gastoElectronico+=pg.monto;
        detGastos.push({fecha:s.fecha_pago||"",concepto:"💼 "+(s.concepto_extra?"Aguinaldo":"Sueldo")+" — "+(s.empleado_nombre||""),medio:pg.medio||"",monto:pg.monto,tipo:esEf?"efectivo":"electronico",cruzado:false});
      });
    });
    ...
    adelantosSueldo.filter(function(a){return a.local===lid&&a.fecha&&a.fecha.substring(0,7)===mesFiltro;}).forEach(function(a){
      // Un adelanto también se puede haber pagado con varios medios
      repartirMedios(a,parseFloat(a.monto||0)).forEach(function(pg){
        var esEf=(pg.medio||"").toLowerCase().includes("efectivo");
        if(esEf)gastoEfectivo+=pg.monto;else gastoElectronico+=pg.monto;
        detGastos.push({fecha:a.fecha,concepto:"⏳ Adelanto sueldo — "+(a.empleado_nombre||""),medio:pg.medio||"",monto:pg.monto,tipo:esEf?"efectivo":"electronico",cruzado:false});
      });
    });
    ...
    sueldosADescontar.forEach(function(s){
      var sm=s.estado==="parcial"?parseFloat(s.monto_parcial||0):parseFloat(s.monto||0);
      repartirMedios(s,sm).forEach(function(pg){
        procesarPagoDetalle((pg.medio||"").toLowerCase(),pg.monto,lid);
      });
    });
  ...
  return(
    <div style={{fontFamily:"'Inter',sans-serif"}}>
      ...
      {/* VISTA COMPARATIVA — pantalla completa */}
      {vistaComparativa&&(
        <div>
          <div style={{overflowX:"auto"}}>
            <table style={{width:"100%",borderCollapse:"collapse",fontSize:11,minWidth:500}}>
              <thead>
                <tr style={{borderBottom:"2px solid #1A1A1A"}}>
                  ...
                  <th style={{textAlign:"right",padding:"8px",color:"#F0EDE8",fontWeight:700,minWidth:100}}>Grupo</th>
                </tr>
              </thead>
              <tbody>
                {/* Ventas brutas */}
                ...
                <div style={{background:"#0A0A0A",borderRadius:10,padding:"12px"}}>
                  ...
                  {d.diasCierre===0?(
                    ...
                    <div>
                      ...
                      {(d.retiros>0||d.retirosModMonto>0||d.egresos>0||d.iibbEgreso>0||d.iibbManual>0||d.comisionEgreso>0||d.comisionManual>0)&&(
...
function PanelSueldos(p){
  ...
  var [formSueldo,setFormSueldo]=useState({empleado_id:"",empleado_nombre:"",local:"l1",periodo:mesCurrent,fecha_pago:hoy,monto:"",monto_convenio:"",monto_sin_convenio:"",monto_parcial:"",medio_pago:"",estado:"pendiente",convenio:"sin_convenio",pagos:[{medio:"",monto:""}],notas:""});
  ...
  // ── Medios de pago del sueldo (se puede repartir entre varias cuentas) ──
  function montoPagadoSueldo(){
    return formSueldo.estado==="parcial"
      ?(parseFloat(formSueldo.monto_parcial)||0)
      :((parseFloat(formSueldo.monto_convenio)||0)+(parseFloat(formSueldo.monto_sin_convenio)||0)||parseFloat(formSueldo.monto)||0);
  }
  function pagosSueldo(){
    var filas=(formSueldo.pagos||[]).filter(function(pg){return pg.medio;});
    if(filas.length===1&&!(parseFloat(filas[0].monto)>0))return [{medio:filas[0].medio,monto:montoPagadoSueldo()}];
    return filas.filter(function(pg){return parseFloat(pg.monto)>0;}).map(function(pg){return{medio:pg.medio,monto:parseFloat(pg.monto)};});
  }
  function totalPagosSueldo(){return pagosSueldo().reduce(function(a,pg){return a+pg.monto;},0);}
  function pagosSueldoCuadran(){var t=montoPagadoSueldo();return t<=0||Math.abs(totalPagosSueldo()-t)<0.01;}
  function setPagoSueldo(idx,campo,valor){
    setFormSueldo(function(f){
      var n=[...(f.pagos||[])];
      n[idx]={...n[idx],[campo]:valor};
      return{...f,pagos:n};
    });
  ...
  function doSaveSueldo(){
    ...
    var pagosValidos=(formSueldo.estado==="pagado"||formSueldo.estado==="parcial")?pagosSueldo():[];
    if((formSueldo.estado==="pagado"||formSueldo.estado==="parcial")&&pagosValidos.length===0){alert("Seleccioná al menos un medio de pago.");return;}
    mesesACargar.forEach(function(periodo){
      ...
      var s={id:sid,empleado_id:formSueldo.empleado_id,empleado_nombre:formSueldo.empleado_nombre,local:formSueldo.local,periodo:periodo,fecha_pago:formSueldo.fecha_pago,monto:montoFinal,monto_convenio:montoConvenio,monto_sin_convenio:montoSinConvenio,convenio:formSueldo.convenio||"sin_convenio",monto_parcial:montoParcial,medio_pago:pagosValidos.length>0?pagosValidos[0].medio:(formSueldo.medio_pago||""),estado:formSueldo.estado,pagos:pagosValidos,notas:formSueldo.notas,usuario:usuario,created_at:new Date().toISOString()};
      onSaveSueldo(s);
    ...
    setFormSueldo({empleado_id:"",empleado_nombre:"",local:"l1",periodo:mesCurrent,fecha_pago:hoy,monto:"",monto_convenio:"",monto_sin_convenio:"",monto_parcial:"",medio_pago:"",estado:"pendiente",convenio:"sin_convenio",pagos:[{medio:"",monto:""}],notas:""});
  }
              ...
              {(formSueldo.estado==="pagado"||formSueldo.estado==="parcial")&&(
                <div>
                  ...
                  {(formSueldo.pagos||[]).map(function(pago,idx){return(
                    <div key={idx} style={{display:"flex",gap:6,marginBottom:6,alignItems:"center"}}>
                      <select value={pago.medio} onChange={function(e){setPagoSueldo(idx,"medio",e.target.value);}} style={{padding:"9px 12px",borderRadius:8,border:"1px solid #2A2A2A",background:"#0F0F0F",color:"#F0EDE8",fontFamily:"'Inter',sans-serif",fontSize:12,width:"100%",boxSizing:"border-box"}}>
                        <option value="">-- Seleccioná --</option>
                        {GRUPOS_MEDIOS_SUELDOS.map(function(g){return(
                          <optgroup key={g} label={"── "+g+" ──"}>{MEDIOS_SUELDOS.filter(function(m){return m.g===g;}).map(function(m){return <option key={m.v} value={m.v}>{m.v}</option>;})}</optgroup>
                        );})}
                      </select>
...
async function sbSaveAdelanto(adelanto) {
  try {
    var h = {...SH, "Prefer": "resolution=merge-duplicates,return=representation"};
    var r = await fetch(SURL + "/rest/v1/adelantos", { method: "POST", headers: h, body: JSON.stringify(adelanto) });
    if(!r.ok){
      var err = await r.text();
      console.error("sbSaveAdelanto error:", r.status, err);
      return /pagos/.test(err)
        ? "Falta la columna \"pagos\" en la tabla adelantos. Corré el ALTER TABLE del README."
        : err;
    }
    return null;
  } catch(e) { return e&&e.message?e.message:String(e); }
...
async function sbSaveGasto(gasto) {
  try {
    ...
    var payload = {
      ...
      area: gasto.area||gasto.categoria||"",
      categoria: gasto.categoria||gasto.area||"",
        ...
        <div style={{padding:"14px 20px",maxWidth:900,margin:"0 auto"}}>
          ...
          {esSofia&&modulo==="admin"&&vista==="egresos"&&(
            <PanelEgresos gastos={gastos} usuario={cu.nombre} proveedores={proveedores}
              ...
              onSaveEgresoSueldo={function(g){sbSaveGasto(g);setGastos(function(prev){var f=prev.filter(function(x){return x.id!==g.id;});return[g,...f];});}}
              adelantos={adelantos}
              onSaveAdelanto={function(a){
                sbSaveAdelanto(a).then(function(err){if(err)alert("No se pudo guardar el adelanto en la base:\n\n"+err);});
                setAdelantos(function(prev){var f=prev.filter(function(x){return x.id!==a.id;});return[a,...f];});
              }}
